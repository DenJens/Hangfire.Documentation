...
.. admonition:: Obsolete since 1.5
   ...
   You are not required to have additional configuration to support multiple background processing servers in the same process since Hangfire 1.5, so just skip this article. Server identifiers are now generated using GUIDs, so all the instance names are unique.
