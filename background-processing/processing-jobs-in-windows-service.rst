...
To start using Hangfire in a Windows Service, you will need to install Hangfire packages to your console application first. So, use your Package Manager Console window to install it:
...
If you are new to Windows Services in .NET projects, it is always better to google about them first, but for quick-start scenario you will need only to add an installer and optionally configure it. To perform this step just go back to the design view of the service class, right click on it and choose the ``Add Installer`` menu item.

.. image:: add-installer.png
   :alt: Adding installer to Windows Service project
   :align: center

Then build your project, install your Windows Service and run it. If it fails, try looking at your Windows Event Viewer for recent exceptions.
